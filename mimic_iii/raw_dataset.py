...
class MIMICDataset(object):
    ...
    def _sort_admissions(self):
        ...
        for patient_id in self.mimic_data.keys():
            ...
            self.mimic_data[patient_id]['patient']['ethnicity'] = last_admit_obj['hadm_ethnicity']  # 가장 마지막 방문 때의 종교
            self.mimic_data[patient_id]['patient']['insurance'] = last_admit_obj['hadm_insurance']  # 가장 마지막 방문 때의 보험 여부
            self.mimic_data[patient_id]['patient']['marital_status'] = last_admit_obj['hadm_marital_status']  # 가장 마지막 방문 때의 결혼 여부
